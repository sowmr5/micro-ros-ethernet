...
void setup() {
  ...
  uint8_t mac[6];
  get_teensy_mac(mac);
  byte teensy_mac[] = {mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]};

  byte local_mac[] = { 0xAA, 0xBB, 0xCC, 0xEE, 0xDD, 0xFF };
  IPAddress local_ip(192, 168, 1, 177);
  IPAddress agent_ip(192, 168, 1, 88);
  size_t agent_port = 8888;

  set_microros_native_ethernet_transports(teensy_mac, local_ip, agent_ip, agent_port);
